...
VAULT_TOKEN_FILE=/home/vagrant/.vault-snapshot-token
...
SNAPSHOT_FILE="/opt/vault/snapshots/snapshot-$(date +"%Y-%m-%d-%H%M%S").snap"
vault operator raft snapshot save $SNAPSHOT_FILE

# Delete old snapshots to keep only the desired number of snapshots
SNAPSHOTS=($(ls -t /opt/vault/snapshots/*.snap))
...
if [ $NUM_SNAPSHOTS_FOUND -gt $NUM_SNAPSHOTS ]; then
  ...
  SNAPSHOT_FILES_TO_DELETE=($(ls -t /opt/vault/snapshots/*.snap | tail -n $NUM_SNAPSHOTS_TO_DELETE))
